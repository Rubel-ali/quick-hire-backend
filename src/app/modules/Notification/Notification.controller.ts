...
const sendNotificationToUser = catchAsync(async (req: Request, res: Response) => {
  const { deviceToken, title, body, studentId } = req.body;

  if (!deviceToken || !title || !body || !studentId) {
  ...
  await notificationService.sendNotification(deviceToken, title, body, studentId);
...
const readNotificationByUserIdController = catchAsync(async (req: any, res: Response) => {
  const  studentId  = req.user?.id;
  const notifications = await notificationService.readNotificationByUserId(studentId);
  sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
